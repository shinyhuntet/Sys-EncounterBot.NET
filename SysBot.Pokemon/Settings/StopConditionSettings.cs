...
namespace SysBot.Pokemon
{
    public class StopConditionSettings
    {
        ...
        public static bool EncounterFound<T>(T pk, int[] targetminIVs, int[] targetmaxIVs, StopConditionSettings settings, IReadOnlyList<string>? naturelist, IReadOnlyList<string>? marklist, Nature SyncNature = Nature.Random) where T : PKM
        {
            ...
            if (issync == false)
            {
                if (naturelist is not null && naturelist.Count > 0)
                {
                    ...
                    {
                        Nature nat;
                        if (Enum.TryParse(text.ToTitleCase(nature.Replace(" ", "")), out nat))
                            if (nat == (Nature)pk.Nature)
                                nat_check = true;
                        else
                            Console.WriteLine($"this nature {nature} is not able to be concerted to nature object !");
                    }
                    if (!nat_check)
                        return false;
                }
            ...
            {
                if (SyncNature != (Nature)pk.Nature)
                    return false;
            }
